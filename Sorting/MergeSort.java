...
public class MergeSort {
	...
	public static void Sort(int[] arr) {
		...
		// calling sort individually in left and right subarrays so that dividing of
		// elements reaches to the point when only one element is left
		Sort(left);
		Sort(right);
	...
	public static void Merge(int[] left, int[] right, int[] res) {
		...
		// we need three pointers: one for left array, one for right array and last for
		// updating elements in final array
		int i = 0, j = 0, k = 0;

		// i and j both must statisfy the condition that they both should be less then
		// their total
		while (i < n && j < m) {
			if (left[i] < right[j]) {
				res[k] = left[i];
